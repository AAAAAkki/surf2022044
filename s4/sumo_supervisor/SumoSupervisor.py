...
from RealworldVehicle import RealworldVehicle
...
class SumoSupervisor (Supervisor):
    ...
    def get_initial_vehicles(self):
        ...
        for i in range(0, self.vehiclesLimit):
            ...
            node = self.getFromDef(defName)
            if node:
                # self.realworldVehicle[i] = Vehicle(node)
                # self.realworldVehicle[i].name.setSFString("REALWORLD vehicle %i" % self.vehicleNumber)
    ...
    def update_realworld_vehicles(self, xOffset, yOffset):
        """Update the position of all the vehicles controlled by RealWorld in SUMO."""
        for i in range(0, self.realworldNumber):
            if self.realworldVehicle[i].is_on_road(xOffset, yOffset, self.maxRealworldVehicleDistanceToLane, self.net):
                self.realworldVehicle[i].update_position(self.getTime(), self.net, self.traci, self.sumolib, xOffset, yOffset)
            else:
                         # the controlled vehicle is not on any road
                         # => we remove it from sumo network
                if self.realworldVehicle[i].name in self.traci.vehicle.getIDList():
                    self.traci.vehicle.remove(self.realworldVehicle[i].name)       
    ...
    def run(self, port, disableTrafficLight, directory, step, rotateWheels,
        ...
        self.maxRealworldVehicleDistanceToLane = 15
